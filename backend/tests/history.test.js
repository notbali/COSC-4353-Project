...
describe("History Routes - Full Coverage", () => {
  beforeEach(() => {
    ...
    Object.keys(mockUserCredentials).forEach(
      (key) => delete mockUserCredentials[key]
    );
  ...
  describe("GET /events", () => {
    ...
    it("should handle events with no volunteers", async () => {
      ...
      expect(res.body.futureEvents[0].assignedVolunteerNames).toBe(
        "Unassigned"
      );
    });
  ...
  describe("DELETE /volunteer-history/:userId/:eventId", () => {
    it("should successfully remove volunteer from event", async () => {
      ...
      const res = await request(app).delete(
        `/api/volunteer-history/${userId}/${eventId}`
      );
    ...
    it("should return 404 if volunteer registration not found", async () => {
      const res = await request(app).delete(
        "/api/volunteer-history/nonexistent/nonexistent"
      );
    ...
    it("should handle database errors", async () => {
      ...
      VolunteerHistory.findOneAndDelete = jest
        .fn()
        .mockRejectedValue(new Error("Database error"));

      const res = await request(app).delete(
        "/api/volunteer-history/user_1/event_1"
      );
